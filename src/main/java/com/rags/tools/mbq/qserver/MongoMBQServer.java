...
public class MongoMBQServer extends AbstractMBQueueServer {
    ...
    public MongoMBQServer(MBQDataStore mbqDataStore, PendingQMap<IdSeqKey> pendingQMap, MBQStatsCollector statsCollector) {
    ...
    private static MBQueueServer createAndInitialize(QConfig.ServerConfig config) {
        ...
        return new MongoMBQServer(new MongoMBQDataStore(config), new InMemoryPendingIdSeqKeyQMap(), getStatsCollector(config.getStatsCollectorClass()));
    }
